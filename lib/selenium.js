...
export class AdForms extends Selenium {
  ...
  async execute(_file) {
    this.spinner.start(Logger.color(`[ADForms]: Uploading file: ${_file.substring(_file.lastIndexOf("/") + 1)}`, "cyan"));
    ...
    await this.waitForElementAndClick(By.xpath("//*[contains(text(), 'Add clicktag')]"));
    await delay(100);
