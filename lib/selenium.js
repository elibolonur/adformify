...
export class Task {
  ...
  async run() {
    for (const filePath of this.fileArray) {
      const adForm = new AdForm(filePath);
...
export class SeleniumRunner {
  constructor(_fileList, _parallelTasks = 1) {
    this.fileArray = chunkArray(_fileList, _parallelTasks);
    this.taskList = [];
  ...
  init() {
    ...
    this.fileArray.forEach(fileList => {
      this.taskList.push(new Task(fileList));
    });
  }
